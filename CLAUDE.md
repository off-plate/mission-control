...
- **Light only.** There is no dark mode and no `data-theme` anywhere in the CSS. Do not write docs, tokens or review notes that assume one; if dark mode is ever wanted, it is a project, not an afterthought.
