...
- One vault folder mirrors one Notes folder, both ways: `OBSIDIAN.md`. It is a launchd job on his Mac (`tools/obsidian-sync.mjs`), not part of the app, and it touches nothing in `src/`. Its `bodyHash` must stay byte for byte identical to the one in `src/sync-merge.ts`, or every ordinary Obsidian save reads as a divergence in the app; `tools/obsidian-test.mjs` asserts that.
