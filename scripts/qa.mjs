...
await t('plan commit', async () => {
  await page.goto(`http://localhost:4180${BASE}/#/plan`, { waitUntil: 'networkidle' })
  await page.locator('.pickrow .checkbox').first().click()
  await page.locator('.radio').first().click()
  await page.screenshot({ path: `${OUT}/flow-plan.png` })
  await page.getByRole('button', { name: 'Commit the day' }).click()
  await page.waitForTimeout(300)
  if (!page.url().includes('today')) throw new Error('did not navigate to today')
