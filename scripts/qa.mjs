...
await step('people: the header opens it, a contact turns someone green, and a circle can be shown alone', async () => {
  ...
  for (const [name, tier] of [['Gate Mum', 'Inner circle'], ['Gate Accountant', 'Business']]) {
    await page.getByRole('button', { name: 'Add person', exact: true }).click()
    await page.locator('.pp-dialog input').first().fill(name)
    await page.locator('.pp-add-circle .dd-trigger').click()
    await page.locator('.pp-add-circle .dd-option', { hasText: tier }).click()
  ...
  await page.locator('.pp-circle-pick .dd-trigger').click()
  await page.locator('.pp-circle-pick .dd-option', { hasText: 'Business' }).click(); await page.waitForTimeout(300)
  ...
  await page.locator('.pp-circle-pick .dd-trigger').click()
  await page.locator('.pp-circle-pick .dd-option', { hasText: 'All circles' }).click(); await page.waitForTimeout(300)
  if (await page.locator('.pp-atom.is-dim').count()) throw new Error('choosing All circles did not show everyone again')
  /* A click on the ring itself still shows that circle alone, and a second click shows everyone. */
  const ringAt = await page.evaluate(() => { const g = document.querySelector('.pp-tier[data-tier=business] .pp-tier-ring'); const m = g.getScreenCTM(); for (let a = 0; a < 360; a += 5) { const x = Math.cos(a * Math.PI / 180) * 450, y = Math.sin(a * Math.PI / 180) * 450; const sx = m.a * x + m.e, sy = m.d * y + m.f; const el = document.elementFromPoint(sx, sy); if (el && el.classList.contains('pp-tier-hit')) return { x: sx, y: sy } } return null })
  if (!ringAt) throw new Error('no clickable spot on the Business ring')
  await page.mouse.click(ringAt.x, ringAt.y); await page.waitForTimeout(300)
  if ((await page.locator('.pp-atom.is-dim').count()) !== 1) throw new Error('clicking the Business ring did not show it alone')
  await page.mouse.click(ringAt.x, ringAt.y); await page.waitForTimeout(300)
  if (await page.locator('.pp-atom.is-dim').count()) throw new Error('a second ring click did not show everyone again')
  ...
  await page.locator('.pp-link-who .dd-trigger').click()
  await page.locator('.pp-link-who .dd-option', { hasText: 'Gate Mum' }).click()
  await page.locator('.pp-link-add input').fill('client')
  await page.locator('.pp-link-add button[type=submit]').click(); await page.waitForTimeout(300)
  await page.keyboard.press('Escape'); await page.waitForTimeout(500)
...
await step('cookie jar: one nav on the ladder, the flywheel, Health and Why, and give up works from any of them', async () => {
  ...
  await nav().getByRole('button', { name: 'Momentum' }).click(); await page.waitForTimeout(500)
  ...
  await nav().getByRole('button', { name: 'Momentum' }).click(); await page.waitForTimeout(600)
