/* The helmet, his own file.

   I drew one by hand first and it was a geometric approximation. He sent the
   real thing, so the approximation is gone: the path data below is his,
   unchanged, only rewired to take its colour from the button (`currentColor`)
   instead of carrying a hardcoded black.

   It is a single filled silhouette with the eye slots cut out by the even-odd
   rule, which means the eyes cannot be lit on their own the way my drawn one
   did. So the WHOLE helmet carries the state: accent when the mode is on,
   quiet when it is off. The icon is still the indicator, which was the point.

   The artwork is portrait (79 x 123), so it is sized by height and lets its
   width follow, rather than being squashed into a square. */
export function Helmet({ lit }: { lit: boolean }) {
  return (
    <svg
      viewBox="0 0 78.89 122.88"
      height="17"
      width="11"
      fill="currentColor"
      fillRule="evenodd"
